...
describe('Client e2e test', () => {
  ...
  const clientSample = {
    clientName: 'Loan integrated',
    clientEmail: 'VVT;:@uLm.5Ax+',
    clientPhone: '48266',
    clientAddress: 'c salmon calculating',
  };
  ...
  describe('new Client page', () => {
    ...
    it('should create an instance of Client', () => {
      cy.get(`[data-cy="clientName"]`).type('quantifying cultivate c').should('have.value', 'quantifying cultivate c');

      cy.get(`[data-cy="clientEmail"]`).type('q@w!qf!k.k').should('have.value', 'q@w!qf!k.k');

      cy.get(`[data-cy="clientPhone"]`).type('5').should('have.value', '5');

      cy.get(`[data-cy="clientAddress"]`)
        .type('back-end Trinité-et-Tobago methodologies')
        .should('have.value', 'back-end Trinité-et-Tobago methodologies');
